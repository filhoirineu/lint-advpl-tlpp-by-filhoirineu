...
// find contiguous block of declaration lines starting with 'Local' (first occurrence)
let start = -1, end = -1;
for (let i=0;i<lines.length;i++){
  if (/^\s*(Local|Private|Static|Default)\b/i.test(lines[i])){ start = i; break; }
}
if (start===-1){ console.error('No declaration block found'); process.exit(2); }
for (let j=start;j<lines.length;j++){
  if (!/^\s*(Local|Private|Static|Default)\b/i.test(lines[j])){ end = j-1; break; }
}
...
function runAlign(blockLines){
  // replicate previous align behavior for the block
  const items = [];
  const assignRe = /^(\s*)([\s\S]*?)(\s*(?::=|\+=|-=|\*=|\/=)\s*)([\s\S]*?)(\s*(\/\/.*))?$/;
  for (const seg of blockLines){
    const am = assignRe.exec(seg);
    if (am){
      const innerIndent = am[1]||''; const left = (am[2]||'').trimEnd(); const op=(am[3]||'').trim(); const right=(am[4]||'').trimStart(); const comment=(am[5]||'').trim();
      items.push({ kind:'assign', orig:seg, prefix:'', innerIndent, left, op, right, comment });
      continue;
    }
...
const sorted = runSort(block);
const aligned = runAlign(sorted);
// post-adjust AS and := to same column in test output
let alignedFinal = aligned.slice();
{
  let target = 0;
  for (const l of alignedFinal) {
    const op = l.indexOf(':=');
  ...
  if (target > 0) {
    for (let i = 0; i < alignedFinal.length; i++) {
      const l = alignedFinal[i];
      const op = l.indexOf(':=');
      const asIdx = l.toLowerCase().indexOf(' as ');
      if (op >= 0 && op < target) {
        const pad = target - op;
        alignedFinal[i] = l.slice(0, op) + ' '.repeat(pad) + l.slice(op);
      } else if (asIdx >= 0 && asIdx < target) {
        const pad = target - asIdx;
        alignedFinal[i] = l.slice(0, asIdx) + ' '.repeat(pad) + l.slice(asIdx);
      }
    }
  }
}
...
// write outputs into tools/tmp for inspection
const outdir = path.join(__dirname,'tmp'); if (!fs.existsSync(outdir)) fs.mkdirSync(outdir,{recursive:true});
writeFile(path.join(outdir,'before.block.txt'), before);
writeFile(path.join(outdir,'sorted.block.txt'), sorted.join('\n'));
writeFile(path.join(outdir,'aligned.block.txt'), aligned.join('\n'));
